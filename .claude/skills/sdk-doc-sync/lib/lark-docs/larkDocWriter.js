...
const {
    BLOCK_ID_TO_NAME,
    LANGUAGE_ID_TO_NAME,
} = require('../../src/document-ir/block-registry.js')
...
class larkDocWriter {
    ...
    async __board(board, indent) {
        ...
        // Add metadata comment for round-trip conversion
        const metadata = `<!-- feishu-block: board, token: ${board.token} -->`;

        if (this.skip_image_download) {
            return `${metadata}\n![${board.token}](${root}/${board["token"]}.png)`;
        }
    ...
    async __iframe(block) {
        ...
        const iframe_url = iframe.component.url;
        const iframe_type = iframe.component.iframe_type;

        // Add metadata comment for round-trip conversion
        const createMetadata = (caption) => {
            return `<!-- feishu-block: iframe, url: ${encodeURIComponent(iframe_url)}, type: ${iframe_type}, caption: ${caption} -->`;
        };

        const existing_iframe = this.iframes.find(x => x.block_id === block_id)
        if (existing_iframe) {
            const metadata = createMetadata(existing_iframe.caption);
            return `${metadata}\n![${existing_iframe.caption}](${root}/${existing_iframe.caption}.png "${existing_iframe.caption}")`;
        }
        ...
        if (iframe['component']['iframe_type'] !== 8) {
            ...
            const metadata = createMetadata(caption);
            return `${metadata}\n![${caption}](${root}/${caption}.png "${caption}")`;
        } else {
            try {
                const url = new URL(decodeURIComponent(iframe.component.url))
                const key = url.pathname.split('/')[2]
    ...
    async __sheet(sheet, indent) {
        ...
        // Add metadata comment to distinguish sheets from regular tables
        const rows = values.length;
        const cols = values[0]?.length || 0;
        const metadata = `<!-- feishu-block: sheet, rows: ${rows}, cols: ${cols} -->\n`;

        var result = ' '.repeat(indent) + metadata + ' '.repeat(indent) + "<table>" + "\n";
    ...
    __block_types() {
        return Array.from(
            { length: Math.max(...Object.keys(BLOCK_ID_TO_NAME).map(Number)) },
            (_, index) => BLOCK_ID_TO_NAME[index + 1] ?? null,
        )
    }
