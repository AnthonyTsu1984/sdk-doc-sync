...
class PythonScanner extends BaseScanner {
    ...
    _extractMethods(lines, classLineIdx, className, filePath) {
        ...
        for (let i = classLineIdx + 1; i < lines.length; i++) {
            ...
            // Check for start of a def statement (supports async def)
            const defStart = line.match(/^\s+(?:async\s+)?def\s+(\w+)\s*\(/);
            ...
            const sigMatch = fullSignature.match(/^\s*(?:async\s+)?def\s+(\w+)\s*\(([^)]*(?:\([^)]*\)[^)]*)*)\)\s*(?:->\s*(.+?))?\s*:/);
