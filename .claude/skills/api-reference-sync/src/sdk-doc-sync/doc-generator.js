...
class DocGenerator {
    ...
    generate(symbol) {
        switch (symbol.kind) {
            ...
            case 'interface':
                ...
                return this._functionScaffold(symbol);
            case 'command':
                return this._cliCommandScaffold(symbol);
            case 'method':
            case 'function':
                return this._functionScaffold(symbol);
            default:
    ...
    _title(symbol) {
        if (symbol.kind === 'class' || symbol.kind === 'enum' || symbol.kind === 'struct' || symbol.kind === 'interface' || symbol.kind === 'command') {
            return symbol.name;
        }
        return `${symbol.name}()`;
    }
    ...
    /**
     * Convert imperative CLI description to "This operation ..." form.
     * "List all clusters." → "This operation lists all clusters."
     * "Get details of a cluster." → "This operation gets details of a cluster."
     * Handles multi-sentence: only transforms the first sentence.
     */
    _cliDescription(desc) {
        if (!desc || desc.startsWith('<!--')) return desc;

        // Already starts with "This operation" — no change
        if (desc.startsWith('This operation')) return desc;

        // Split into first sentence and rest, handling abbreviations like "etc."
        // Match sentence boundary: period followed by space+uppercase, or period at end
        const match = desc.match(/^(.+?\.)\s+([A-Z].*)/s);
        let first, rest;
        if (match) {
            first = match[1];
            rest = match[2];
        ...
        // Extract the leading verb and conjugate to 3rd person
        const verbMatch = first.match(/^(\w+)\s/);
        if (!verbMatch) return desc;

        const verb = verbMatch[1];
        const conjugated = this._conjugateVerb(verb);
        first = `This operation ${conjugated}${first.slice(verb.length)}`;

        return rest ? `${first} ${rest}` : first;
    }

    /**
     * Conjugate an imperative verb to 3rd person singular present.
     * "List" → "lists", "Get" → "gets", "Create" → "creates", etc.
     */
    _conjugateVerb(verb) {
        const lower = verb.toLowerCase();
        // Irregular
        if (lower === 'get') return 'gets';
        if (lower === 'has') return 'has';
        // -sh, -ch, -x, -ss, -o → +es
        if (/(?:sh|ch|x|ss|o)$/.test(lower)) return lower + 'es';
        // consonant + y → ies
        if (/[^aeiou]y$/.test(lower)) return lower.slice(0, -1) + 'ies';
        // -e → +s
        if (lower.endsWith('e')) return lower + 's';
        return lower + 's';
    }

    /**
     * Formalize a terse CLI parameter description.
     * "cluster ID" → "Specifies the cluster ID."
     * "items per page" → "Specifies the items per page."
     * Already-formal descriptions (starting with uppercase + verb) are left alone.
     */
    _cliParamDescription(desc) {
        if (!desc || desc.startsWith('<!--')) return desc;
        // Already starts with "Specifies" or similar formal prefix
        if (/^(Specifies|Sets|Enables|Disables|Indicates|Controls)\b/.test(desc)) return desc;
        // Already a proper sentence (starts with uppercase) — just ensure period
        if (/^[A-Z]/.test(desc)) {
            return desc.endsWith('.') ? desc : `${desc}.`;
        }

        // Lowercase fragment (from JSON models) — add "Specifies the " prefix
        let result = desc;
        if (!/^(the|a|an)\b/i.test(result)) {
            result = `the ${result}`;
        }
    ...
    _cliCommandScaffold(symbol) {
        let md = '';

        // Description — convert imperative "List all clusters." to "This operation lists all clusters."
        const rawDesc = symbol.docstring || `<!-- TODO: Brief description of ${symbol.parentClass} ${symbol.name} -->`;
        md += `${this._cliDescription(rawDesc)}\n\n`;

        // Dedicated-only note
        if (symbol.dedicatedOnly) {
            md += `> **Note:** This command is available for Dedicated clusters only.\n\n`;
        }
        ...
        if (symbol.params && symbol.params.length > 0) {
            md += `**OPTIONS:**\n\n`;
            for (const p of symbol.params) {
                // Flag name: --long, -s or just --long
                const flagName = p.shorthand ? `${p.name}, ${p.shorthand}` : p.name;
                ...
                // Description line — formalize with "Specifies" prefix
                let desc = p.description || `<!-- TODO: Description of ${p.name} -->`;
                desc = this._cliParamDescription(desc);

                // Append choices if present
                if (p.choices && p.choices.length > 0) {
                    desc += ` Choices: ${p.choices.map(c => `\`${c}\``).join(', ')}.`;
                }

                // Append default if present (skip false for boolean flags — it's implied)
                if (p.default !== null && p.default !== undefined && p.default !== false) {
                    desc += ` Default: \`${p.default}\`.`;
                }

                // Append requiredUnless/requiredWhen
                if (p.requiredUnless) {
                    desc += ` Required unless \`--${p.requiredUnless}\` is provided.`;
                }
                if (p.requiredWhen) {
                    const conditions = Object.entries(p.requiredWhen)
                        .map(([k, v]) => `\`--${k}\` is \`${v}\``)
                        .join(' and ');
                    desc += ` Required when ${conditions}.`;
                }
        ...
        if (symbol.examples && symbol.examples.length > 0) {
            ...
            md += `\`\`\`bash\n# TODO: Usage example\nzilliz ${symbol.parentClass.toLowerCase()} ${symbol.name}\n\`\`\`\n`;
        }
