...
- REST fragment production is a local, deterministic operation. Data-plane collections come from pinned Milvus revisions; control-plane collections come from pinned zilliz-cloud base/head revisions and the explicit service allowlist. It must not read control-plane truth from Feishu or infer publication versions from OpenAPI `info.version` or `/v1`/`/v2` paths.
- REST source adapters may extract source routes, but they must never guess an internal-to-public control-plane mapping. When constants, gateways, request-shape transformations, split/combined operations, or missing controller ownership prevent an exact automatic mapping, follow the agent mapping investigation in `sdk-rest.md`. This is work for the agent, not a request for the user to confirm routes. Keep the service blocked until the agent records source-backed mapping evidence; path similarity alone is not approval.
- PR intake from `milvus-io/web-content` is read-only discovery (`bin/sdk-pr-scan.js`, see `sdk-pr-sync.md`). It pins the PR merge commit and the SDK target tag, verifies every page change against SDK source, and must emit a `validateReleaseScope`-passing artifact before the standard candidate pipeline runs. Only MERGED PRs are solidifiable; Feishu-local edits diverging from a PR surface at the grouping review gate and are decided page by page — intake never silently overwrites them. Languages without a track (csharp, rust) produce inventory-only artifacts (`NO_FEISHU_TRACK`).
...
- Versioned Drive trees follow the delta model (global, all languages): the older track's tree (e.g. `v2.6.x`) holds the full page inventory, while the newer track's tree (e.g. `v3.0.x`) holds only the pages whose content changed relative to the older track. Unchanged interfaces keep one shared document in the older tree and both tracks' records point at it. When an updated interface's category directory does not exist in the newer tree, create it as a sibling under the newer tree root before copying, and repoint the newer track's category VirtualNode to the new directory. Never patch a shared cross-track document in place when the interface changed — copy first, then patch the copy. The executed transition follows a fixed DAG — create the category folder, then copy-patch and repoint the interface record, and only after the copied document is verified repoint the category VirtualNode as a distinct downstream action — and ends with a post-write tree-delta verification that refetches both tracks before acceptance. [api.versioned-tree-delta]
...
- Prove verbatim pages landed intact: a merged-PR page's normalized content digest is attested into the approved plan, in-place patch strategies refuse cross-type block pairings and demand a rebuild when shapes diverge (REBUILD_REQUIRED_SHAPE_MISMATCH), and execution completes only after the refetched raw_content compares line-for-line against the attested content through the declared canonicalization, with the outcome persisted in the journal and acceptance receipt. [api.pr-verbatim-content]
- Keep the governed document inventory closed: every document under a tracked release folder is either a record's Docs target or referenced by a percent-decoded page block link; reconciliation reports unreferenced documents, callout blocks carrying auto-populated empty children, and reviewed contexts whose frozen digest or live raw_content disagrees with their content — as detect-only findings that never authorize disposal. [api.governed-document-inventory]
- Write pages that conform to their language's declared layout profile: per-language layout rules live only as versioned data in the renderer layout profiles (never duplicated in enforcement code), one language-neutral checker evaluates the declared structural rules — builder signature prefixes, single-request H3, example H3, deprecation callout shape — at the operational verify step and over live pages in reconciliation, and rules a language does not declare do not apply to it. [api.sdk-page-layout]
