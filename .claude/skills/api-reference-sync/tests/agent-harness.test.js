...
test('agent harness accepts bounded release-scout artifact and command trace', () => {
  ...
  const validation = validateAgentArtifact({
    ...
    releaseScope,
    commandLog: [
      'node .claude/skills/api-reference-sync/bin/sdk-release-scout.js --language python --sdk-name pymilvus --track v2.6.x --json --output tmp/python-v26.json',
      'node .claude/skills/api-reference-sync/bin/sdk-doc-sync.js --release-scope tmp/python-v26.json --dry-run --json',
    ],
  });
...
test('agent harness rejects full scanner dumps as final artifacts', () => {
  ...
  const validation = validateAgentArtifact({
    caseFixture,
    releaseScope,
    commandLog: ['node .claude/skills/api-reference-sync/bin/sdk-doc-sync.js --dry-run --json'],
  });
