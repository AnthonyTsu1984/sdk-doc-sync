...
// runtime-enforced fixture against the production inventory/planner/executor/
// session-store modules and fails on unexecuted registry fixtures.
...
test('pre-write refusals prove zero-writer-call blocking', async () => {
  const { fixturesById } = registryFixtures();
  const { scenarios } = require('./conformance-fixtures/invariant-scenarios');
  for (const fixtureId of [
    'authoring-writepath-digest-mismatch-zero-writes',
    'authoring-writepath-target-drift-zero-writes',
  ]) {
