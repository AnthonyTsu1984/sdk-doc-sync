...
test('api-reference-sync never instructs routine version sync to delete historical documents', () => {
  const content = skill('api-reference-sync');
...
for (const name of ['verified-doc-authoring', 'localized-doc-sync', 'procedure-code-sync', 'api-reference-sync']) {
...
test('large skills route detailed operational material into direct references', () => {
  const sdk = skill('api-reference-sync');
  assert.ok(sdk.split(/\r?\n/).length < 300, 'api-reference-sync/SKILL.md must stay under 300 lines');
  ...
  for (const reference of [
    ...
    assert.equal(fs.existsSync(path.join(REPO_ROOT, '.claude', 'skills', 'api-reference-sync', reference)), true);
  }

  const verifier = skill('doc-code-verify');
  assert.doesNotMatch(verifier, /^## Manta Runtime Verification$/m);
  assert.match(verifier, /references\/manta-runtime\.md/);
  assert.equal(
    fs.existsSync(path.join(REPO_ROOT, '.claude', 'skills', 'doc-code-verify', 'references', 'manta-runtime.md')),
    true,
  );

  const patchSkill = skill('procedure-code-sync');
  ...
  const development = fs.readFileSync(
    path.join(REPO_ROOT, '.claude', 'skills', 'localized-doc-sync', 'references', 'development-alignment.md'),
    'utf8',
  );
