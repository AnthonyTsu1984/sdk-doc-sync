...
const OLD = "new MilvusClient({ address: 'localhost:19530', token: 'root:Milvus' })";
const NEW = "new MilvusClient({\n    address: 'localhost:19530',\n    token: 'root:Milvus',\n})";
...
async function main() {
    console.log(`Reformat MilvusClient constructor${DRY_RUN ? ' (DRY RUN)' : ''}\n`);
    ...
    for (const rec of methodRecords) {
        ...
        try {
            ...
            for (const block of blocks) {
                ...
                console.log(`  [${title}]`);
                updates.push({ block_id: block.block_id, update_text_elements: { elements: [{ text_run: { content: fixed } }] } });
                patched++;
            }
            if (updates.length > 0 && !DRY_RUN) {
                await feishuAPI('PATCH', `/open-apis/docx/v1/documents/${docId}/blocks/batch_update`, { requests: updates });
                await delay();
            } else if (updates.length === 0) noMatch++;
        } catch (err) { console.error(`  ERROR [${title}]: ${err.message}`); errors++; }
    }
    console.log(`\nDone. Patched: ${patched}, no match: ${noMatch}, errors: ${errors}`);
}
