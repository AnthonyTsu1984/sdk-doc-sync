...
4. **Post-actions after bulk create/update:** run `add-type-links.js`, `fix-leading-spaces.js`, and `post-fix-links.js` in that order. Their docx block mutations route through the governed `DocxBlockWriter` — exception governance binds a run manifest naming the working-tree fingerprint — and they remain runtime-quarantined as legacy-live: a run requires an unexpired exception in `.claude/skills/doc-ops-core/expected-changes.json` **and** `DOC_OPS_ALLOW_LEGACY_LIVE=1`; such a run is exception-admitted (source-bound but **not harness-guaranteed**) and must never advance accepted scan state.
