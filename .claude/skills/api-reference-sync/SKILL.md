...
- Classify each type as `standalone`, `method_owned`, or `ambiguous`; embed every method-owned request, response, result, task, info, iterator, descriptor, transport, and wrapper type in every owner document.
- A public class alone is not standalone evidence. Ambiguous ownership blocks planning; a standalone type requires an explicit reviewed standalone exception.
...
```bash
node .claude/skills/api-reference-sync/scripts/build-reviewed-release-context.js \
  --release-scope tmp/sdk-release-scout/python-v26.json \
