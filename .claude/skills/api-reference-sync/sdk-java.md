...
- Phase 3 validates every linked inline-code reference in the simulated Feishu rich-text run: the visible content has no backticks, linked API identifiers automatically require `text_element_style.inline_code: true`, and the link matches the recorded canonical target. Encode the canonical lookups as `requiredLinkedInlineCode` checks in the patch preview and as `linkedInlineCodeRequirements` beside the document blocks in the operational manifest. Any mismatch is `planning_blocked`.
- Set `language: "java"` on the operational manifest and include refetched live blocks for every touched Java document. Every document token in `publicationAccess` must have a matching `javaDocuments` entry; partial evidence is a completion blocker.
...
- For direct Feishu XML patches, follow the `lark-doc` XML nesting rules. A linked inline-code type is `<a href="canonical-doc-url"><code>TypeName</code></a>`; a bold numeric value is `<b>134,217,728</b>`; and a builder bullet is `<li><code>signature</code><p>description</p></li>` semantically, with the description verified as a child block after writing. Do not assume Markdown such as a link whose label contains backticks will preserve both styles; inspect the converted rich block or use XML.
