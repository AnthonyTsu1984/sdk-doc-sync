...
2. Extract reference content.
   - For Feishu docs, use `.claude/skills/api-reference-sync/bin/export-doc.js <doc-token-or-url> <tmp-file.md>` or `scripts/feishu-doc.js get-blocks <doc-id>` when block IDs matter.
...
4. Verify against source.
   ...
   - For REST APIs, compare source handlers/DTOs with `.claude/skills/api-reference-sync/specs/openapi-milvus.json` or `openapi-cloud.json` as appropriate.
...
7. Patch the target Feishu page.
   ...
   - Use `.claude/skills/api-reference-sync/scripts/feishu-doc.js patch <target-doc-id> <draft.md> --strategy smart|replace|append`.
...
Reuse the existing `.claude/skills/api-reference-sync` dependencies and scripts. Do not add a new package.json, install a second converter stack, or duplicate Feishu auth logic for this skill.

Use these existing assets when needed:

- Feishu export: `.claude/skills/api-reference-sync/bin/export-doc.js`
- Feishu push/patch/blocks/folder/bitable CLI: `.claude/skills/api-reference-sync/scripts/feishu-doc.js`
- Markdown to Feishu converter: `.claude/skills/api-reference-sync/src/markdown-to-feishu.js`
- Feishu to Markdown converter: `.claude/skills/api-reference-sync/src/feishu-to-markdown.js`
- Bitable writer/reader support: `.claude/skills/api-reference-sync/src/sdk-doc-sync/bitable-writer.js`
- Milvus and Zilliz OpenAPI specs: `.claude/skills/api-reference-sync/specs/openapi-milvus.json` and `openapi-cloud.json`
- Code example verification: `.claude/skills/doc-code-verify/scripts/verify-feishu-doc-code.js`
