...
1. Resolve and read the Feishu doc.
   - If a Feishu URL/token is supplied, use the `lark-doc` skill or `.claude/skills/api-reference-sync/scripts/feishu-doc.js get-blocks <doc-id>` to fetch blocks.
...
6. Patch the Feishu doc.
   ...
   - Use `.claude/skills/api-reference-sync/docs/converters/patch-document.md` only when a whole-document Markdown patch is safer.
7. Verify after patching.
   ...
   - Run `.claude/skills/doc-code-verify/scripts/verify-feishu-doc-code.js` when feasible, at least in parse/compile mode for changed languages.
...
- If the doc is versioned or linked from a bitable, follow the version-aware update rules in `.claude/skills/api-reference-sync/SKILL.md` before writing.
